...
static const char *def_btrfs_subvol_home = "no";
...
static const char *user_selinux_range;
#endif			  /* WITH_SELINUX */
...
static bool sgr_locked;
#endif
#ifdef ENABLE_SUBIDS
static bool is_sub_uid;
static bool is_sub_gid;
static bool sub_uid_locked;
static bool sub_gid_locked;
static uid_t sub_uid_start; /* New subordinate uid range */
static gid_t sub_gid_start; /* New subordinate gid range */
#endif			  /* ENABLE_SUBIDS */
static bool pw_locked;
static bool gr_locked;
static bool spw_locked;
static char **user_groups;  /* NULL-terminated list */
static long sys_ngroups;
static bool do_grp_update;  /* group files need to be updated */
...
#define DBTRFS_SUBVOL_HOME  "BTRFS_SUBVOLUME_HOME"
...
get_defaults(struct option_flags *flags)
{
	...
	*/
	...
	/*
	* Default BTRFS subvolume home directory creation
	*/
	else if (streq(buf, DBTRFS_SUBVOL_HOME)) {
	if (streq(ccp, ""))
	ccp = "no";

	def_btrfs_subvol_home = xstrdup(ccp);
	}
	}
	(void) fclose(fp);
getdef_err:
...
static void show_defaults(void)
{
	...
	printf("LOG_INIT=%s\n", def_log_init);
	printf(DBTRFS_SUBVOL_HOME "=%s\n", def_btrfs_subvol_home);
}
...
set_defaults(void)
{
	...
	bool  out_btrfs_subvol_home = false;
	char  buf[1024];
	char  *new_file = NULL;
	...
	*/
	...
	out_log_init = true;
	} else if (!out_btrfs_subvol_home && streq(buf, DBTRFS_SUBVOL_HOME)) {
	fprintf(ofp, DBTRFS_SUBVOL_HOME "=%s\n", def_btrfs_subvol_home);
	out_btrfs_subvol_home = true;
	} else {
	fprintf(ofp, "%s=%s\n", buf, val);
	}
	}
...
skip:
	...
	*/
	...
	unlink(buf);
	if ((link(default_file, buf) != 0) && (errno != ENOENT)) {
	fprintf(stderr,
	...
	*/
	...
	#endif
	SYSLOG((LOG_INFO,
	"useradd defaults: GROUP=%u, HOME=%s, SHELL=%s, INACTIVE=%ld, EXPIRE=%s, SKEL=%s, CREATE_MAIL_SPOOL=%s, LOG_INIT=%s, "
	DBTRFS_SUBVOL_HOME "=%s",
...
{
	FILE *usageout = (status != E_SUCCESS) ? stderr : stdout;
...
static void process_flags(int argc, char **argv, struct option_flags *flags)
{
	...
	*/
	...
	#ifdef WITH_BTRFS
	if (!subvolflg && streq(def_btrfs_subvol_home, "yes")) {
	subvolflg = true;
	}
	#endif
}
...
static void create_mail(struct option_flags *flags)
{
	...
	if ((spool == NULL) && (getdef_str("MAIL_FILE") == NULL)) {
